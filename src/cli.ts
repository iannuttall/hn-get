...
import { HackerNewsClient, type LiveListName, type ScrapedListName, type SortMode } from "./hn.js";
...
program
  ...
  .action(withErrorHandling(async (query: string | undefined, options) => {
    const response = await client.search({
      ...
      sort: options.sort as SortMode,
    });
    await printOutput(response, options.format as OutputFormat);
  }));
...
program
  ...
  .action(withErrorHandling(async (id: string, options) => {
    if (options.feed) {
      await printOutput(
        await client.itemComments(parseItemId(id), {
          author: options.author,
          limit: parsePositiveInt(options.limit, "--limit", { max: 100 }),
        }),
    ...
    await printOutput(item, options.format as OutputFormat);
  }));
...
program
  ...
  .action(withErrorHandling(async (id: string, query: string | undefined, options) => {
    await printOutput(
      await client.itemComments(parseItemId(id), {
        query,
        author: options.author,
...
program
  ...
  .option("-l, --limit <count>", "feed results to return when --feed is set, max 100", "20")
  .action(withErrorHandling(async (username: string, options) => {
    if (options.feed) {
      await printOutput(
        await client.submitted(username, {
          limit: parsePositiveInt(options.limit, "--limit", { max: 100 }),
          type: "all",
        }),
        options.format as OutputFormat,
    ...
    await printOutput(await client.user(username, parseUserOptions(options)), options.format as OutputFormat);
  }));
...
program
  ...
  .option("-l, --limit <count>", "results to return, max 100", "20")
  .action(withErrorHandling(async (username: string, options) => {
    await printOutput(await client.threads(username, parsePositiveInt(options.limit, "--limit", { max: 100 })), options.format as OutputFormat);
  }));
...
program
  ...
  .addOption(createOutputOption())
  .option("-l, --limit <count>", "results to return, max 100", "20")
  .action(withErrorHandling(async (idOrUsername: string, options) => {
    await printOutput(await client.replies(idOrUsername, parsePositiveInt(options.limit, "--limit", { max: 100 })), options.format as OutputFormat);
  }));
...
program
  ...
  .action(withErrorHandling(async (username: string, options) => {
    await printOutput(await client.favorites(username, parsePositiveInt(options.limit, "--limit", { max: 500 })), options.format as OutputFormat);
  }));
...
program
  ...
  .addOption(createOutputOption())
  .option("-l, --limit <count>", "results to return, max 100", "20")
  .action(withErrorHandling(async (kind: string | undefined, options) => {
    const normalizedKind = normalizeWhoIsHiringKind(kind);
    await printOutput(await client.whoIsHiring(normalizedKind, parsePositiveInt(options.limit, "--limit", { max: 100 })), options.format as OutputFormat);
  }));

program.parseAsync();

function createOutputOption(): Option {
  return new Option("-f, --format <format>", "output format")
    .choices(["json", "compact", "ndjson", "markdown", "text"])
    .default("json");
}

function parseUserOptions(options: { submitted?: boolean; submittedLimit?: string }): { includeSubmitted?: boolean; submittedLimit?: number } {
  return {
...
function addSearchFeedCommand(
  command: string,
  description: string,
  config: {
    tags: string;
    aliases?: string[];
    defaultType?: "story" | "comment" | "poll" | "job" | "all";
...
function addLiveListCommand(command: string, description: string, list: LiveListName): void {
  program
    .command(command)
    .description(description)
    .addOption(createOutputOption())
    ...
    .action(withErrorHandling(async (options) => {
      await printOutput(await client.liveList(list, parsePositiveInt(options.limit, "--limit", { max: 500 })), options.format as OutputFormat);
    }));
}

function addScrapedListCommand(command: string, description: string, list: ScrapedListName): void {
  program
    .command(command)
    .description(description)
    .addOption(createOutputOption())
    .option("-l, --limit <count>", "items to return, max 500", "30")
    .action(withErrorHandling(async (options) => {
      await printOutput(await client.scrapedList(list, parsePositiveInt(options.limit, "--limit", { max: 500 })), options.format as OutputFormat);
    }));
}

function withErrorHandling<T extends unknown[]>(handler: (...args: T) => Promise<void>): (...args: T) => Promise<void> {
  return async (...args: T) => {
    try {
      await handler(...args);
    } catch (error) {
...
function normalizeWhoIsHiringKind(kind: string | undefined): "jobs" | "hired" | "freelance" | "all" {
