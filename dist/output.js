...
export async function printOutput(value, format) {
    if (format === "compact") {
...
function printSearchText(response) {
...
function printResultText(result) {
...
async function renderSearchMarkdown(response) {
...
async function renderResultMarkdown(result, index) {
    const title = result.title ?? result.storyTitle ?? result.textPlain?.slice(0, 120) ?? "(untitled)";
...
async function renderItemMarkdown(item, index) {
...
async function renderCommentsMarkdown(comments, depth) {
    const parts = [];
    for (const comment of comments) {
...
function isSearchResponse(value) {
    return Boolean(value && typeof value === "object" && "results" in value && Array.isArray(value.results));
}
function isNormalizedItem(value) {
